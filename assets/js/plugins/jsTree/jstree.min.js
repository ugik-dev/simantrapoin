					...
					? (this.each(function () {
							...
							return (
								...
								g ||
									...
									a(this).data("jstree", new a.jstree.create(this, c)),
								((g && !d) || c === !0) && (f = g || !1),
								null !== f && f !== b ? !1 : void 0
							);
					  }),
			...
			(a.jstree.core.prototype = {
				...
				get_path: function (a, b, c) {
					...
					var d,
						...
						f = [];
					for (
						f.push(c ? a.id : a.text), d = 0, e = a.parents.length;
						e > d;
						d++
					)
					...
					if (b.state.loaded) {
						for (
							...
							for (f = 0, g = b.parents.length; g > f; f++)
								this._model.data[
									b.parents[f]
								].children_d = a.vakata.array_remove_item(
				...
				load_all: function (a, b) {
					...
					var c = [],
						...
						g;
					for (
						a.state && !a.state.loaded && c.push(a.id), f = 0, g = e.length;
						g > f;
						f++
					)
										...
										d === !1 && c.call(this, !1),
											this[
												"string" == typeof d
													? "_append_html_data"
													: "_append_json_data"
											](b, "string" == typeof d ? a(d) : d, function (a) {
												c.call(this, a);
											});
									}, this)
							  )
				...
				_parse_model_from_html: function (b, c, d) {
					...
					for (i in this._model.default_state)
						this._model.default_state.hasOwnProperty(i) &&
					...
					if (
						...
						for (i in h.data.jstree)
							h.data.jstree.hasOwnProperty(i) &&
								(h.state[i] = h.data.jstree[i]);
					(j = b.children("a").children(".jstree-themeicon")),
						j.length &&
							(h.icon = j.hasClass("jstree-themeicon-hidden")
								? !1
								: j.attr("rel")),
							...
							? (j.each(
									a.proxy(function (b, c) {
										(e = this._parse_model_from_html(a(c), h.id, d)),
											...
											h.children.push(e),
											f.children_d.length &&
												(h.children_d = h.children_d.concat(f.children_d));
									}, this)
							  ),
				...
				_redraw: function () {
					...
					this._model.force_full_redraw &&
						...
						null !== f &&
							...
							c &&
							c.length &&
							c.children(".jstree-anchor")[0] !== document.activeElement
				...
				draw_children: function (a) {
					...
					if (
						...
						(a = a[0]),
						b.children.length && b.state.loaded)
					) {
						for (
				...
				redraw_node: function (b, c, d, e) {
					...
					if (
						...
						h = a.inArray(
							f.id,
							null === g ? p["#"].children : p[f.parent].children
						);
					}
				...
				open_all: function (a, b, c) {
					...
					(c = c || d),
						(g = this),
						(d = this.is_closed(a)
							? d.find(".jstree-closed").addBack()
							: d.find(".jstree-closed")),
						d.each(function () {
							g.open_node(
								this,
								function (a, d) {
						...
						0 === c.find(".jstree-closed").length &&
							this.trigger("open_all", { node: this.get_node(c) });
				},
				...
				is_selected: function (a) {
					return (
						(a = this.get_node(a)), a && "#" !== a.id ? a.state.selected : !1
					);
				},
													...
													(g.is_loaded(h)
														? (g.is_closed(h) && g.open_node(h, !1, 0),
														  c &&
																c.core &&
																c.core.open &&
																a.vakata.array_remove_item(c.core.open, h))
														: (g.is_loading(h) ||
																g.open_node(
																	h,
						...
						for (h = 0, i = b.parents.length; i > h; h++)
							(f = a.inArray(g[j], this._model.data[b.parents[h]].children_d)),
								-1 !== f &&
									(this._model.data[
										b.parents[h]
									].children_d = a.vakata.array_remove(
				...
				move_node: function (c, d, e, f, g, h) {
					...
					if (
						((k = (c.parent || "#").toString()),
						(m =
							e.toString().match(/^(before|after)$/) && "#" !== d.id
						...
						return this.copy_node(c, d, e, f, g)
							? (n && n.delete_node(c), !0)
							: !1;
					switch (
						("#" === d.id &&
					...
					if (c.parent === m.id) {
						...
						for (
							...
							for (
								p = [],
									v = n._model.data[c.parents[r]].children_d,
									t = 0,
									u = v.length;
								u > t;
								t++
						...
						for (
							n._model.data[k].children = a.vakata.array_remove_item(
								n._model.data[k].children,
								c.id
							),
								r = 0,
								s = m.parents.length;
							...
							this._model.data[m.parents[r]].children_d = this._model.data[
								m.parents[r]
							].children_d.concat(q);
						for (p = [], r = 0, s = m.children.length; s > r; r++)
							p[r >= e ? r + 1 : r] = m.children[r];
						for (
							p[e] = c.id,
								m.children = p,
							...
							(this._model.data[c.children_d[r]].parents = this._model.data[
								c.children_d[r]
							].parents.slice(0, -1 * v)),
								Array.prototype.push.apply(
									this._model.data[c.children_d[r]].parents,
									q
								);
						("#" === k || "#" === m.id) && (this._model.force_full_redraw = !0),
							this._model.force_full_redraw ||
								(this._node_changed(k), this._node_changed(m.id)),
							h || this.redraw();
					}
					return (
						f && f.call(this, c, m, e),
				...
				copy_node: function (c, d, e, f, g, h) {
					...
					if (a.isArray(c)) {
						for (c = c.slice(), i = 0, j = c.length; j > i; i++)
							(l = this.copy_node(c[i], d, e, f, g, !0)),
								l && ((d = l), (e = "after"));
						return this.redraw(), !0;
					}
					if (((c = c && c.id ? c : this.get_node(c)), !c || "#" === c.id))
					...
					if (
						((o = r
							? r.get_json(c, { no_id: !0, no_data: !0, no_state: !0 })
							: c),
						!o)
					)
						return !1;
					if (
						(o.id === !0 && delete o.id,
						(o = this._parse_model_from_json(o, q.id, q.parents.concat())),
						!o)
					)
						return !1;
					for (
						...
						this._model.data[q.parents[m]].children_d = this._model.data[
							q.parents[m]
						].children_d.concat(k);
					for (k = [], m = 0, n = q.children.length; n > m; m++)
						k[m >= e ? m + 1 : m] = q.children[m];
					return (
						(k[e] = l.id),
						(q.children = k),
				...
				cut: function (b) {
					...
					for (h = 0, i = b.length; i > h; h++)
						(g = this.get_node(b[h])), g && g.id && "#" !== g.id && c.push(g);
					return c.length
						? ((d = c),
						  (f = this),
						  (e = "move_node"),
				...
				(this.bind = function () {
					...
					(this._undetermined = function () {
						...
						for (b = 0, c = f.length; c > b; b++)
							d[f[b]] && d[f[b]].parents && (g = g.concat(d[f[b]].parents));
						for (
							this.element
								.find(".jstree-closed")
						...
						return (
							this.settings.checkbox.tie_selection &&
								(this.settings.checkbox.whole_node ||
									a(d.target).hasClass("jstree-checkbox")) &&
								(d.ctrlKey = !0),
							this.settings.checkbox.tie_selection ||
							(!this.settings.checkbox.whole_node &&
								!a(d.target).hasClass("jstree-checkbox"))
								...
								if (
									...
									(!this.settings.dnd.touch ||
										("selected" === this.settings.dnd.touch &&
											!a(b.currentTarget).hasClass("jstree-clicked")))
								)
									return !0;
								var c = this.get_node(b.target),
									d =
										this.is_selected(c) && this.settings.drag_selection
					...
					.on("dnd_move.vakata.jstree", function (f, g) {
						if (
							...
							g &&
								g.data &&
								g.data.jstree &&
								(!g.event.target.id || "jstree-marker" !== g.event.target.id))
						) {
							...
							if (h && h._data && h._data.dnd)
								if (
									(e.attr(
										...
										"jstree-" +
											h.get_theme() +
											(h.settings.core.themes.responsive
												? " jstree-dnd-responsive"
												: "")
									),
									g.helper
									...
									(g.event.target !== h.element[0] &&
										g.event.target !== h.get_container_ul()[0]) ||
												...
												for (r = !0, s = 0, t = g.data.nodes.length; t > s; s++)
													if (
														...
														"move_node" === u &&
															...
															((w = h.get_node(o)),
															v > a.inArray(g.data.nodes[s], w.children) &&
																(v -= 1)),
														(r =
															r &&
															((h &&
																h.settings &&
																h.settings.dnd &&
																h.settings.dnd.check_while_dragging === !1) ||
																h.check(
																	u,
						...
						if ((d && clearTimeout(d), g && g.data && g.data.jstree)) {
							...
							if (b) {
								for (h = 0, i = g.data.nodes.length; i > h; h++)
									(j[h] = g.data.origin
										? g.data.origin.get_node(g.data.nodes[h])
										: g.data.nodes[h]),
								...
								(h = a(g.event.target).closest(".jstree")),
									h.length &&
										...
										"check" === c.error &&
										((h = h.jstree(!0)),
										h && h.settings.core.error.call(this, c));
						}
					})
			...
			(function (a) {
				...
				a.vakata.dnd = {
					...
					_trigger: function (b, c) {
						...
						(d.event = c),
							a(document).triggerHandler("dnd_" + b + ".vakata", d);
					},
					_get: function () {
						...
						if (
							...
							return (
								...
								b.scroll_e ||
									...
									(k = d.scrollLeft()),
									f > g &&
										c.pageY - j < a.vakata.dnd.settings.scroll_proximity &&
										(b.scroll_t = -1),
									f > g &&
										g - (c.pageY - j) <
											a.vakata.dnd.settings.scroll_proximity &&
										(b.scroll_t = 1),
									h > i &&
										c.pageX - k < a.vakata.dnd.settings.scroll_proximity &&
										(b.scroll_l = -1),
									h > i &&
										i - (c.pageX - k) <
											a.vakata.dnd.settings.scroll_proximity &&
								...
								a.proxy(function (b, c) {
									...
									if (
										...
										for (d = 0, e = g.length; e > d; d++)
											h = h.concat(this.get_node(g[d]).parents);
										(h = a.vakata.array_remove_item(
							...
							d || g === !1
								? ((this._data.search.str = c),
								  ...
								  a.each(this._model.data, function (a, b) {
										b.text &&
											((f.search_callback &&
												f.search_callback.call(this, c, b)) ||
												(!f.search_callback && h.search(b.text).isMatch)) &&
											(!f.search_leaves_only ||
												(b.state.loaded && 0 === b.children.length)) &&
			...
			(function (a) {
				a.vakata.search = function (a, b, c) {
					...
					return (
						...
						(k = function (b) {
							if (
								((b = c.caseSensitive ? b : b.toLowerCase()),
								a === b || -1 !== b.indexOf(a))
							)
							...
							for (
								-1 !== n &&
									((m = Math.min(j(0, n), m)),
									(n = b.lastIndexOf(a, d + g)),
									-1 !== n && (m = Math.min(j(0, n), m))),
									n = -1,
									e = 0;
								g > e;
								e++
			...
			(a.jstree.plugins.unique = function (c, d) {
				(this.check = function (b, c, e, f, g) {
					...
					(this.create_node = function (c, e, f, g, h) {
						if (!e || e.text === b) {
							if ((null === c && (c = "#"), (c = this.get_node(c)), !c))
