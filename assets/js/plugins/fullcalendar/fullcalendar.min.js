...
!(function (a) {
	...
	function O(a, b) {
		...
		for (
			c = 0;
			c < _a.length && ((d = _a[c]), (e = P(d, a, b)), !(e >= 1 && ha(e)));
			c++
		);
		return d;
	}
	...
	function Qa(c, e) {
		...
		function h() {
			c.addClass("fc"),
				...
				U.options.handleWindowResize &&
					((aa = ja(s, U.options.windowResizeDelay)), a(window).resize(aa));
		}
		function i() {
		...
		function l(b, c) {
			ha++,
				...
				!$ &&
					...
					$.setElement(
						a("<div class='fc-view fc-" + b + "-view' />").appendTo(Y)
					),
					W.activateButton(b)),
				$ &&
		...
		function B() {
			var a = U.getNow();
			a.isWithin($.intervalStart, $.intervalEnd)
		...
		function S(a) {
			...
			if (1 === c) {
				...
				if ("businessHours" === b)
					return void (
						$ && ($.unrenderBusinessHours(), $.renderBusinessHours())
					);
		...
		U.bindOptions(
			...
			function (a, b, c, d, e, g, h) {
				...
				"iso" === h && (h = "ISO"),
					("ISO" !== h && "local" !== h && "function" != typeof h) ||
						(V._fullCalendar_weekCalc = h),
					ba && f(ba);
			}
		),
			(U.defaultAllDayEventDuration = b.duration(
				U.options.defaultAllDayEventDuration
			)),
			(U.defaultTimedEventDuration = b.duration(
				U.options.defaultTimedEventDuration
			)),
			(U.moment = function () {
				var a;
				return (
			...
			(U.getDefaultEventEnd = function (a, b) {
				...
				return (
					a
						? c.stripTime().add(U.defaultAllDayEventDuration)
						: c.add(U.defaultTimedEventDuration),
		...
		var W,
			...
			ia = [];
		(ba =
			null != U.options.defaultDate
				? U.moment(U.options.defaultDate).stripZone()
				: U.getNow()),
			...
			(U.isHeightAuto = function () {
				return (
					"auto" === U.options.contentHeight || "auto" === U.options.height
				);
			}),
	...
	function Ua() {
				...
				if (c === b._fetchId && "rejected" !== b._status) {
					if (((b._status = "resolved"), d))
						for (e = 0; e < d.length; e++)
							(f = d[e]), (g = i ? f : C(f, b)), g && $.push.apply($, G(g));
					h();
				}
			});
		}
		...
		function i(b, c) {
			...
			for (d = 0; d < f.length; d++) {
				if (
					((e = f[d].call(S, b, T.clone(), U.clone(), S.options.timezone, c)),
					e === !0)
				)
					return;
			...
			if (g)
				...
				if (h) {
					...
					o && (n[o] = T.format()),
						p && (n[p] = U.format()),
						S.options.timezone &&
							"local" != S.options.timezone &&
							(n[q] = S.options.timezone),
						S.pushLoading(),
						a.ajax(
							a.extend({}, Eb, b, {
								data: n,
		...
		function y(a, b) {
			...
			if (f) {
				for (c = G(f), d = 0; d < c.length; d++)
					(e = c[d]),
						e.source || (b && (X.events.push(e), (e.source = X)), $.push(e));
				return W($), c;
			}
			return [];
		}
		...
		function C(c, d) {
			...
			if (
				...
				f && ((f = S.moment(f)), f.isValid() || (f = null)),
					(g = c.allDay),
					void 0 === g &&
						(g = ba(d ? d.allDayDefault : void 0, S.options.allDayDefault)),
					D(e, f, g, h);
			}
		...
		function E(a) {
			F(a),
				...
				a.end ||
					(S.options.forceEventDuration
						? (a.end = S.getDefaultEventEnd(a.allDay, a.start))
						: (a.end = null));
		}
		function F(a) {
			null == a.allDay &&
				...
				a.allDay
					...
					: (a.start.hasTime() || (a.start = S.applyTimezone(a.start.time(0))),
					  a.end &&
							!a.end.hasTime() &&
							(a.end = S.applyTimezone(a.end.time(0))));
		}
		function G(b, c, d) {
			var e,
				f,
			...
			return (
				...
				(f = {
					start: b._start.clone(),
						...
						? b._end.clone()
						: S.getDefaultEventEnd(b._allDay, b._start),
					allDay: c.allDay,
				}),
				E(f),
				...
				a.each(b, function (b, j) {
					...
					(k = {
						...
						c
							...
							: f &&
							  !l.end &&
							  (l.end = S.getDefaultEventEnd(l.allDay, l.start)),
		...
		function K(b, c, d) {
			var e, f;
			return (
				d && ((e = a.extend({}, d, c)), (f = G(C(e))[0])), f ? J(b, f) : O(b)
			);
		}
		function O(a) {
		...
		function Q(a) {
			return "businessHours" === a
				? S.getCurrentBusinessHourEvents()
				: "object" == typeof a
				? G(C(a))
				: A(a);
		}
		...
		var T,
			...
			$ = [];
		a.each(
			(S.options.events ? [S.options.events] : []).concat(
				S.options.eventSources || []
			),
			function (a, b) {
				var c = k(b);
				c && Y.push(c);
	...
	var lb = (Wa.EmitterMixin = {
				...
				c = {
					...
					listenTo: function (b, c, d) {
						if ("object" == typeof c)
							...
							"string" == typeof c &&
								b.on(c + "." + this.getListenerNamespace(), a.proxy(d, this));
					},
		...
		pb = (Wa.CoordCache = ya.extend({
			...
			isLeftInBounds: function (a) {
				return (
					!this.boundingRect ||
					(a >= this.boundingRect.left && a < this.boundingRect.right)
				);
			},
			isTopInBounds: function (a) {
				return (
					!this.boundingRect ||
					(a >= this.boundingRect.top && a < this.boundingRect.bottom)
				);
			},
		})),
	...
	qb.mixin({
		...
		initAutoScroll: function () {
			...
			(this.isAutoScroll =
				...
				this.isAutoScroll &&
					this.listenTo(a, "scroll", ja(this.handleDebouncedScroll, 100));
		},
					...
					a
						? ((c = { left: v(a), top: w(a) }),
						  ...
						  e &&
								...
								(this.origHit && (b = C(this.origHit, b) || b), (d = E(b))),
					...
					f = new rb(this, {
						...
						interactionEnd: function (e, f) {
							f ||
								(a &&
									!c.isIgnoringMouse &&
									d.triggerDayClick(c.getHitSpan(a), c.getHitEl(a), e),
	...
	tb.mixin({
		...
		bindSegHandlerToEl: function (b, c, d) {
			...
			b.on(c, ".fc-event-container > *", function (b) {
				...
				return !c || e.isDraggingSeg || e.isResizingSeg
					? void 0
					: d.call(e, c, b);
			});
		},
	...
	var ub = (Wa.DayTableMixin = {
		...
		vb = (Wa.DayGrid = tb.extend(ub, {
			...
			queryHit: function (a, b) {
				if (
					this.colCoordCache.isLeftInBounds(a) &&
					this.rowCoordCache.isTopInBounds(b)
				) {
			...
			for (e = 0; e < c.length; e++)
				(f = c[e]),
					(d && !f.dow) ||
						...
						h.push.apply(
							h,
							this.expandEvent(this.buildEventFromInput(f), g.start, g.end)
						));
			return h;
		});
