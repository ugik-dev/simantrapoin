...
(function ($) {
	$.fn.justifiedGallery = function (options) {
		...
		return this.each(function (index, cont) {
			...
			$(cont).append(
				'<div class="jg-loading"><div class="jg-loading-img"></div></div>'
			);
			...
			$(cont)
				...
				.each(function (index, entry) {
					...
					images[index]["src"] =
						typeof $(imgEntry).data("safe-src") != "undefined"
							? $(imgEntry).data("safe-src")
							: $(imgEntry).attr("src");
					images[index]["alt"] = $(imgEntry).attr("alt");
					images[index]["href"] = $(entry).attr("href");
					images[index]["title"] = $(entry).attr("title");
					images[index]["rel"] =
						settings.rel != null ? settings.rel : $(entry).attr("rel");
					images[index]["target"] =
						settings.target != null ? settings.target : $(entry).attr("target");
					images[index]["extension"] = images[index]["src"].match(
						settings.extension
					)[0];
					...
					$(img).load(function () {
						if (images[index]["height"] != settings.rowHeight)
							images[index]["width"] = Math.ceil(
								this.width / (this.height / settings.rowHeight)
							);
					...
					$(img).error(function () {
						$(cont).prepend(
							getErrorHtml(
								"The image can't be loaded: \"" + images[index]["src"] + '"',
								"jg-usedPrefixImageNotFound"
							)
						);
						images[index] = null;
						if (++loaded == images.length) startProcess(cont, images, settings);
		...
		function processesImages($, cont, images, lastRowWidth, settings) {
			...
			$(cont)
				...
				.error(function () {
					$(cont).prepend(
						getErrorHtml(
							"The image can't be loaded: \"" + $(this).attr("src") + '"',
							"jg-resizedImageNotFound"
						)
					);
				})
				.each(function () {
