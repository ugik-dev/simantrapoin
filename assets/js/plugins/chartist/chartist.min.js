			...
			(c.noop = function (a) {
				...
				(c.normalizeDataArray = function (a, b) {
					for (var c = 0; c < a.length; c++)
						if (a[c].length !== b)
							for (var d = a[c].length; b > d; d++) a[c][d] = void 0;
					return a;
				}),
				...
				(c.getBounds = function (a, b, d, e, f) {
					...
					(e || 0 === e) &&
						((j.high = Math.max(e, j.high)), (j.low = Math.min(e, j.low))),
				...
				(c.createChartRect = function (a, b, d) {
					...
					(h = Math.max(h, g + j.left + j.right)),
						(i = Math.max(i, f + j.top + j.bottom));
					var k = {
						padding: j,
						width: function () {
				...
				(c.createLabel = function (a, b, d, e, f, g, h, i, j, k) {
					...
					if (
						...
						l = h.foreignObject(
							n,
							c.extend({ style: "overflow: visible;" }, m)
						);
						...
						(l[k] || 0 === l[k]) &&
							...
							j.showLabel &&
								c.createLabel(
									...
									[
										h.classNames.label,
										h.classNames[a.units.dir],
										h.classNames[j.position],
									],
									g,
									i
								));
			...
			(c.xmlNs = {
				...
				(c.Svg.isSupported = function (a) {
					return b.implementation.hasFeature(
						"www.http://w3.org/TR/SVG11/feature#" + a,
						"1.1"
					);
				});
			var C = {
		...
		(function (a, b, c) {
			...
			function d(a, b, d) {
				c.StepAxis["super"].constructor.call(this, a, b, d),
					(this.stepLength =
						this.axisLength / (d.stepCount - (d.stretch ? 1 : 0)));
			}
			function e(a, b) {
		...
		(function (a, b, c) {
			...
			function d(a) {
				...
				var e = c.createChartRect(this.svg, a, f.padding),
					g = c.getHighLow(d, a),
					h = new c.StepAxis(c.Axis.units.x, e, {
					...
					this.data.series.forEach(
						function (f, g) {
							...
							d[g].forEach(
								function (a, b) {
									var l = {
										x: e.x1 + h.projectValue(a, b, d[g]).pos,
										y: e.y1 - i.projectValue(a, b, d[g]).pos,
									};
									j.push(l.x, l.y),
										k.push({
											value: a,
											valueIndex: b,
											meta: c.getMetaData(f, b),
										});
								}.bind(this)
							);
							var l = {
									...
									n.pathElements.forEach(
										function (d) {
											var e = b[g]
												.elem(
													"line",
													{ x1: d.x, y1: d.y, x2: d.x + 0.01, y2: d.y },
													a.classNames.point
												)
												.attr(
													{ value: d.data.value, meta: d.data.meta },
													c.xmlNs.uri
												);
							...
							if (l.showArea) {
								var p = Math.max(
										Math.min(a.areaBase, i.bounds.max),
										i.bounds.min
									),
		...
		(function (a, b, c) {
			...
			function d(a) {
				...
				var o = this.svg.elem("g").addClass(a.classNames.labelGroup),
					...
					q = a.horizontalBars
						? n.x1 + i.projectValue(0).pos
						: n.y1 - i.projectValue(0).pos,
					r = [];
				c.createAxis(
					k,
					this.data.labels,
									...
									function (j, l) {
										...
										if (
											...
											(t[k.units.pos + "1"] = m[k.units.pos]),
												...
												(t[k.counterUnits.pos + "2"] = a.stackBars
													? r[l]
													: m[k.counterUnits.pos]),
												(o = d[e]
													.elem("line", t, a.classNames.bar)
													.attr(
														{ value: j, meta: c.getMetaData(b, l) },
														c.xmlNs.uri
													)),
				...
				for (
					...
					(i[n] = this.svg.elem("g", null, null, !0)),
						...
						i[n].addClass(
							[
								a.classNames.series,
								o.className || a.classNames.series + "-" + c.alphaNumerate(n),
							].join(" ")
						);
					var p = j + (k[n] / h) * 360;
					p - j === 360 && (p -= 0.01);
					var q = c.polarToCartesian(l.x, l.y, e, j - (0 === n || m ? 0 : 0.2)),
						...
						s = new c.Svg.Path(!a.donut)
							.move(r.x, r.y)
							.arc(e, e, 0, p - j > 180, 0, q.x, q.y);
					...
					if (
						...
						var u = c.polarToCartesian(l.x, l.y, f, j + (p - j) / 2),
							v = a.labelInterpolationFnc(
								this.data.labels ? this.data.labels[n] : k[n],
								n
							);
						if (v || 0 === v) {
							var w = i[n]
