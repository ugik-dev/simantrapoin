	...
	DateRangePicker.prototype = {
		...
		setOptions: function (options, callback) {
			...
			if (
				...
				if (
					...
					this.timeZone =
						moment.tz.zone(options.timeZone).parse(new Date()) * -1; // Offset is positive if the timezone is behind UTC and negative if it is ahead.
				} else {
			...
			this.container
				.removeClass("opensleft opensright")
		...
		inputsChanged: function (e) {
			...
			if (el.attr("name") === "daterangepicker_start") {
				startDate =
					false !== this.minDate && date.isBefore(this.minDate)
						? this.minDate
						: date;
				...
				endDate =
					false !== this.maxDate && date.isAfter(this.maxDate)
						? this.maxDate
						: date;
			}
