...
class UserTempController extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('UserTempModel', 'SecurityModel'));
        $this->load->helper(array('DataStructure', 'Validation'));
        $this->db->db_debug = FALSE;
    }

    public function ShowDetail($id, $username)
    {
        $this->SecurityModel->rolesOnlyGuard(array('kasi_umum', 'kasi_usaha', 'kadin', 'frontoffice', 'admin'));
        // var_dump($this->session->userdata());
        $pageData = array(
            'title' => 'Detail Customer',
            'content' => 'frontoffice/DetailRequestCustomer',
            'breadcrumb' => array(
                'Home' => base_url(),
            ),
        );
    ...
    public function getAllUserTemp()
    {
        ...
        try {
            $this->SecurityModel->userOnlyGuard(TRUE);
            $data = $this->UserTempModel->getAllUserTemp($this->input->POST());
    ...
    public function addUserTemp()
    {
        try {
            ...
            $idUserTemp = $this->UserTempModel->addUserTemp($data);
            $data = $this->UserTempModel->getUserTemp($idUserTemp);
            echo json_encode(array('data' => $data));
        } catch (Exception $e) {
            ExceptionHandler::handle($e);
        }
    ...
    public function editUserTemp()
    {
        try {
            $this->SecurityModel->userOnlyGuard(TRUE);
            $data = $this->input->post();
            $temp = $this->UserTempModel->getUserTemp($data['id_pengiriman']);
            ...
            if (!empty($data['dokumen_permohonanFilename'])) $data['dokumen_permohonan'] = FileIO::genericUpload('dokumen_permohonan', array('pdf'), NULL, $data, '1000');
            $idUserTemp = $this->UserTempModel->editUserTemp($data);
            $data = $this->UserTempModel->getUserTemp($idUserTemp);
            echo json_encode(array('data' => $data));
        } catch (Exception $e) {
            ExceptionHandler::handle($e);
    ...
    {
        try {
            $this->SecurityModel->rolesOnlyGuard(array('admin', 'frontoffice'));
            $id = $this->input->post()['id'];
            $data = $this->UserTempModel->getAllUserTemp(array('id_user_temp' => $id))[$id];
            if ($data['accept'] <> '0') {
                throw new UserException("User sudah ada keputusan!!", USER_NOT_FOUND_CODE);
            }
            $this->UserTempModel->accept($data);
            $filename = 'upload/npwp/' . $data['npwp'];
            if (file_exists($filename)) {
                $newplace   = 'upload/accept_npwp/' . $data['npwp'];
                rename($filename, $newplace);
            }

            $filename = 'upload/ktp/' . $data['ktp'];
            if (file_exists($filename)) {
                $newplace   = 'upload/accept_ktp/' . $data['ktp'];
                rename($filename, $newplace);
            }


            $filename = 'upload/swa/' . $data['swa'];
            if (file_exists($filename)) {
                $newplace   = 'upload/accept_swa/' . $data['swa'];
                rename($filename, $newplace);
            }


            $filename = 'upload/pas_photo/' . $data['pas_photo'];
            if (file_exists($filename)) {
                $newplace   = 'upload/accept_pas_photo/' . $data['pas_photo'];
                rename($filename, $newplace);
            }
            $data = $this->UserTempModel->getAllUserTemp(array('id_user_temp' => $id))[$id];
            echo json_encode(array('data' => $data));
    ...
    public function not_accept()
    {
        try {
            ...
            $this->UserTempModel->not_accept($data);
            $data = $this->UserTempModel->getAllUserTemp(array('id_user_temp' => $id))[$id];
            echo json_encode(array('data' => $data));
        } catch (Exception $e) {
            ExceptionHandler::handle($e);
    ...
    public function word()
    {
        ...
        $idUserTemp = $this->input->get()['id_pengiriman'];
        $data = $this->UserTempModel->getUserTemp($idUserTemp);
        // var_dump($data);
